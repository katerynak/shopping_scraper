...
class CoopSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield scrapy.FormRequest(
            url=self.shop_search_url, formdata=params, callback=self.parse, method="GET"
        )
    ...
    def parse(self, response):
        ...
        names = response.xpath(
            "//h2[starts-with(@class, 'productTitle')]/a/text()"
        ).getall()
        ...
        quantities = response.xpath(
            "//div[starts-with(@class, 'productDescription')]/div/div/text()"
        ).getall()
        product_links = response.xpath("//a[@class='img']/@href").getall()
        id = response.xpath(
            "//article[contains(@class, 'listItem')]/@data-product"
        ).getall()
        ids = []
        for id_item in id:
