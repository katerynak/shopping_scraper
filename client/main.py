...
PRODUCT_COLUMNS = [
    ...
    {"field": "shop_link", "title": "link", "sortable": True},
    {
        "field": "image",
        "title": "image",
    },
...
def search_results(product_name):
    ...
    redis_connection.lpush(os.environ["BROKER_PRODUCTS_INPUT_QUEUE"], product_name)
...
def _get_search_products(search_term: str) -> List[Dict[str, Any]]:
    ...
    products = Product.Product.objects((mongo_visitor.Q(search_term=search_term)))
    ...
    for product in products:
        ...
        for price in product_prices:
            ...
            item[
                "shop_link"
            ] = f'<a href="{str(price.shop_link)}">{str(price.shop)}</a>'
