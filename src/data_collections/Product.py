...
class Product(mongo.DynamicDocument):
    ...
    # shop_ids = mongo.ListField(required=False)
    coop_id = mongo.IntField(required=False)
    ah_id = mongo.IntField(required=False)
    ...
    # brand = mongo.StringField(required=False, max_length=50)
    # ingredients = mongo.ListField(required=False)
